...
function email_func( $atts = array(), $content = '' ) {	
	global $moscourier_options;
	$contact_email = $moscourier_options['contact-email'];
	$html = '';	
	$atts = shortcode_atts( array(
		'offset' => 0,
		'index' => 0,
...
function phone_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $moscourier_options['contact-phone'][$i]);
	    $html .= '" >';
	    $html .= $moscourier_options['contact-phone'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($moscourier_options['contact-phone'] as $phone) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="phone-number">';
			    $html .= '<a class="phoneToShow" href="tel:';
...
function fax_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= preg_replace('/[^0-9]/', '', $moscourier_options['contact-fax'][$i]);
	    $html .= '" >';
	    $html .= $moscourier_options['contact-fax'][$i];  
	    $html .= '</a>';
	    $html .= '</span>';		
	else :
		foreach ($moscourier_options['contact-fax'] as $fax) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="fax-number">';
			    $html .= '<a class="faxToShow" href="tel:';
...
function business_hour_func( $atts = array(), $content = '' ) {
	$html = '';
	global $moscourier_options;
	$contact_hours = $moscourier_options['contact-hour'];
	if ($contact_hours){
...
function address_func( $atts = array(), $content = '' ) {
	...
	if ($atts['index']) :
	    ...
	    $html .= '<span class="address-title">'.$moscourier_options['contact-address'][$i]['title'].'</span>';
		if ($moscourier_options['contact-address'][$i]['map_link']) :
			$html .= '<a class="address-details" href="'.$moscourier_options['contact-address'][$i]['map_link'].'" target="_blank">'.$moscourier_options['contact-address'][$i]['description'].'</a>';
		else :
			$html .= '<span  class="address-details">'.$moscourier_options['contact-address'][$i]['description'].'</span>';
		endif;
	    $html .= '</span>';
	else :
		foreach ($moscourier_options['contact-address'] as $address) :
			if ($n > $atts['offset']) :
			    $html .= '<span class="address address-'.$n.'">';
...
function social_menu_fnc( $atts = array(), $content = '' ) {
	...
	global $moscourier_options;
	$html = '';
	$contact_social = $moscourier_options['contact-social'];
	$contact_address = $moscourier_options['contact-address'];
	$atts = shortcode_atts( array(
		'display' => 'inline',
		'title' => 0,
	), $atts, 'social-menu' );
